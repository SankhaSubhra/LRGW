...
def init_matrix(C1, C2, p, q, loss_fun='square_loss', nx=None, delta = 0.1, epsilon = 1.0):
    ...
    """
    #robust huber 
    def robust_huber_loss(a, b, epsilon, delta):
        def huber(x):
            return nx.where(nx.abs(x) <= delta, 
                            0.5 * x**2, 
                            delta * (nx.abs(x) - 0.5 * delta))
        
        regular_loss = huber(a - b)
        max_loss = nx.maximum(huber(a), huber(b))
        
        return (1 - epsilon) * regular_loss + epsilon * max_loss
    ...
    '''
        ...
        def f1(a):
            return nx.where(nx.abs(a) <= delta, a**2 / (4* delta ), 0.5 * (nx.abs(a) - delta/2))

        def f2(b):
            return nx.where(nx.abs(b) <= delta, b**2 / (4*delta), 0.5 * (nx.abs(b) - delta/2))

        def h1(a):
            return nx.where(nx.abs(a) <= delta, a / (2*delta), 0.5 * nx.sign(a))

        def h2(b):
            return nx.where(nx.abs(b) <= delta, b / (2*delta), 0.5 * nx.sign(b))
    '''
    ...
    elif loss_fun == 'tukey_loss':
        def f1(a):
            # return np.where(nx.abs(a) <= delta, a**2, 0)
            #experiment section
            # return np.where(nx.abs(a) <= delta / 2, a**2, delta ** 2)
            return np.where(nx.abs(a) <= delta, 0.5 * a**2, delta ** 2)

        def f2(b):
            #experiment section
            # return np.where(nx.abs(b) <= delta / 2, b**2, 0)
            return np.where(nx.abs(b) <= delta, 0.5 * b**2, 0)

        def h1(a):
            # return np.where(np.abs(a) <= delta / 2, 2 * a, 0)
            return np.where(np.abs(a) <= delta, a, 0)

        def h2(b):
            # return np.zeros_like(b)
            #experiment section
            # return np.where(np.abs(b) <= delta / 2, b, 0)
            return np.where(np.abs(b) <= delta, 0.5 * b, 0)

    # elif loss_fun == 'huber_loss':
    #     def f1(a):
    #         return nx.where(nx.abs(a) <= delta, 0.5 * a**2, delta * (nx.abs(a)))

    #     def f2(b):
    #         return nx.where(nx.abs(b) <= delta, 0.5 * b**2, delta * (nx.abs(b)))

    #     def h1(a):
    #         return nx.where(nx.abs(a) <= delta, a, delta * 0.5)

    #     def h2(b):
    #         return nx.where(nx.abs(b) <= delta, b, delta)
    
    elif loss_fun == 'huber_loss':
        def f1(a):
            return nx.where(nx.abs(a) <= delta / 2, 0.5 * a**2 / delta, (nx.abs(a)))

        def f2(b):
            return nx.where(nx.abs(b) <= delta / 2, 0.5 * b**2 / delta, (nx.abs(b)))

        def h1(a):
            return nx.where(nx.abs(a) <= delta / 2, a / delta, delta * 0.5)

        def h2(b):
            return nx.where(nx.abs(b) <= delta / 2, b, 1)
    
    elif loss_fun == 'robust_square_loss':
        def f1(a):
            return (1 - epsilon) * a**2 + epsilon * a**2

        def f2(b):
            return (1 - epsilon) * b**2 + epsilon * b**2

        def h1(a):
            return (1 - epsilon) * 2 * a

        def h2(b):
            return 2 * b
        
    elif loss_fun == 'robust_huber_loss':
        def f1(a):
            return robust_huber_loss(a, 0, epsilon, delta)

        def f2(b):
            return robust_huber_loss(0, b, epsilon, delta)

        def h1(a):
            return nx.where(nx.abs(a) <= delta, a, delta * nx.sign(a))

        def h2(b):
            return nx.where(nx.abs(b) <= delta, b, delta * nx.sign(b))
    else:
        raise ValueError(f"Unknown `loss_fun='{loss_fun}'`. Use one of: {'square_loss', 'kl_loss', 'huber_loss', 'robust_square_loss'}.")
