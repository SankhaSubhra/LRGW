...
def gwgrad(C1, C2, G):
    """Compute the gradient of the Gromov-Wasserstein problem."""
    return np.dot(np.dot(C1, G), C2.T) + np.dot(np.dot(C1.T, G), C2)

def sinkhorn_knopp_unbalanced(p, q, M, epsilon, lambda1, lambda2, numItermax=1000, stopThr=1e-9):
    """Solve the unbalanced OT problem using Sinkhorn-Knopp algorithm."""
    K = np.exp(-M / epsilon)
    u = np.ones_like(p)
    v = np.ones_like(q)
    ...
    for _ in range(numItermax):
        ...
        u = (p / Kq) ** (lambda1 / (lambda1 + epsilon))
        v = (q / Kp) ** (lambda2 / (lambda2 + epsilon))
        
        Kp = np.dot(K, u * p)
        Kq = np.dot(K.T, v * q)
        
        if np.max(np.abs(u - uprev)) < stopThr and np.max(np.abs(v - vprev)) < stopThr:
            break
...
def kl_divergence(p, q):
    ...
    Parameters:
    p, q : array_like
        Input distributions. They should sum to 1 and have the same shape.
...
def unbalanced_gromov_wasserstein(C1, C2, p, q, m=None, nb_dummies=1, G0=None,
                               thres=1, numItermax=1000, tol=1e-7,
                               log=False, verbose=False, **kwargs):
    if m is None:
        m = np.min((np.sum(p), np.sum(q)))
    ...
    if log:
        log['partial_gw_dist'] = gwloss_partial(C1, C2, G0)
        log['kl_divergence'] = kl_divergence(p, q)
        return G0[:len(p), :len(q)], log
    else:
        return G0[:len(p), :len(q)]
'''
def unbalanced_gromov_wasserstein(C1, C2, p, q, lambda_reg, div_type='kl', epsilon=1e-3, numItermax=1000, tol=1e-7, log=False, verbose=False):
    """
    Solves the Unbalanced Gromov-Wasserstein problem and returns the OT plan
    
    Parameters:
    ----------
    C1, C2, p, q : as in partial_gromov_wasserstein
    lambda_reg : float
        Regularization parameter for the unbalanced OT
    div_type : str, optional
        Type of divergence to use ('kl' for KL-divergence, 'tv' for Total Variation)
    ...
    ns, nt = len(p), len(q)
    G = np.outer(p, q)  # Initialize transport plan
    ...
    while (err > tol and cpt < numItermax):
        ...
        if div_type == 'kl':
            # KL-divergence (Equation 8)
            G = sinkhorn_knopp_unbalanced(p, q, M, epsilon, lambda_reg, lambda_reg)
        elif div_type == 'tv':
            # Total variation (Equation 9)
            # We approximate TV using a smoothed version
            G = sinkhorn_knopp_unbalanced(p, q, M, epsilon, lambda_reg, lambda_reg, method='sinkhorn_stabilized')
        else:
        ...
        if cpt % 10 == 0:
            ...
            if log:
                log['err'].append(err)
            if verbose:
                print(f'Iteration {cpt}, Error: {err}')
