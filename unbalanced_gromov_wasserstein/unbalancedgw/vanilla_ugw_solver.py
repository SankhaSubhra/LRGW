import torch
from ._vanilla_utils import init_plan, compute_local_cost, \
    log_sinkhorn, exp_sinkhorn


def log_ugw_sinkhorn(a, dx, b, dy, loss, init=None, eps=1.0,
    ...
    for i in range(nits_plan):
        ...
        lcost = compute_local_cost(logpi.exp(), a, dx, b, dy, eps, rho, rho2, loss = loss)
...
def exp_ugw_sinkhorn(a, dx, b, dy, loss, init=None, eps=1.0,
                     rho=float("Inf"), rho2=None,
                     nits_plan=30000, tol_plan=1e-9,
                     nits_sinkhorn=30000, tol_sinkhorn=1e-9,
                     two_outputs=False):
    """Solves the regularized UGW problem, keeps only one plan as output.
    the algorithm is run as much as possible in log-scale.
    ...
    for i in range(nits_plan):
        pi_prev = pi.clone()
        mp = pi.sum()
        ecost = (-compute_local_cost(pi, a, dx, b, dy, eps, rho, rho2, loss) /
                 (mp * eps)).exp()
