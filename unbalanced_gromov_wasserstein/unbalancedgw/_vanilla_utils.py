...
def compute_local_cost(pi, a, dx, b, dy, eps, rho, rho2, loss, delta = 1.0, complete_cost=True):
    ...
    if loss == "square":
        ...
        diff = distxx[:, None] + distyy[None, :] - 2 * distxy
        lcost = torch.where(torch.abs(diff) <= delta,
                                0.5 * diff ** 2,
